-- Add permission for approving balance closure snapshots.

START TRANSACTION;

INSERT INTO permissions (code, description, scope)
SELECT 'balance.approve', 'Approvazione bilancio', 'action'
WHERE NOT EXISTS (
    SELECT 1
    FROM permissions
    WHERE code = 'balance.approve'
);

INSERT INTO profile_permissions (profile_id, permission_id)
SELECT profiles.id, permissions.id
FROM profiles
JOIN permissions ON permissions.code = 'balance.approve'
